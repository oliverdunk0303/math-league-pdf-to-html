...
def add_text_widget_in_gap(page, problem_count, x_start, x_end, baseline_y, height=12, min_width=30, debug=False):
    """
    ...
    rect = fitz.Rect(x_start + 2, baseline_y - (height - 2), max(x_end - 2, x_start + min_width - 2), baseline_y + 2)
...
@click.option('--page_numbers', '-p', default=[1, 2], multiple=True, type=int, help="The page numbers to process.")
...
def convert_number_sense(
    ...
    field_height, min_field_width, min_line_width, debug,
):
            ...
            for line in blank_lines:
                if y_range[0] < line["y"] < y_range[1] and  x_range[0] < line["x1"] < x_range[1]:  # Check if the line is close to the baseline and to the right of the text
                    if not longest_line or abs(line["x1"] - line["x2"]) > abs(longest_line["x1"] - longest_line["x2"]):
                        longest_line = line
            if longest_line:
                add_text_widget_in_gap(page, problem_count, longest_line["x1"], longest_line["x2"], longest_line["y"], height=field_height, min_width=min_field_width, debug=debug)
    ...
    if output_path is None:
        output_path = Path(input_path).with_name(f"{Path(input_path).stem}_filled.pdf")
    elif output_path.endswith(".pdf"):
        output_path = Path(output_path)
    else:
        output_path = Path(output_path) / f"{Path(input_path).stem}_filled.pdf"
